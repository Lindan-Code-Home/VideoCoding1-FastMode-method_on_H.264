...
void encode_one_macroblock_high (Macroblock *currMB)
{
  ...
  {  
    ...
    {   
	  ...
	  for(i_Lindan=1;i_Lindan<max_index;i_Lindan++) 
     { mode=mb_mode_table[i_Lindan]; 
       enc_mb.valid[mode]=0;}
	  //enc_mb.valid[0]=0;
	 //if(currMB->min_rdcost<Tlow) {enc_mb.valid[0]=1;}
	  //if (currMB->min_rdcost>Thigh) 
	  //{ 
		  //NumofIntraLindan++;
	 // enc_mb.valid[I16MB]=1;enc_mb.valid[I4MB]=1; }
	  //if( (currMB->min_rdcost<=Thigh)&&(currMB->min_rdcost>=Tlow))
	  //else 
	  { if(currMB->mb_x>0&&currMB->mb_x<10&&currMB->mb_y>0)
	     {
          b[0]=LofPrevious[11*currMB->mb_y+currMB->mb_x];
	      b[1]=LofCurrent[11*currMB->mb_y+currMB->mb_x-1];
	      b[2]=LofCurrent[11*(currMB->mb_y-1)+currMB->mb_x];
	      b[3]=LofCurrent[11*(currMB->mb_y-1)+currMB->mb_x+1];
		  max_L=b[0];
	    ...
	    if((currMB->mb_y==0)&&(currMB->mb_x==0))
	       {
	        for(i_Lindan=0;i_Lindan<5;i_Lindan++) 
           { mode=mb_mode_table[i_Lindan]; 
             enc_mb.valid[mode]=1;}
	        }
	     if((currMB->mb_x==0)&&(currMB->mb_y>0))
	      {max_L=LofCurrent[11*(currMB->mb_y-1)+currMB->mb_x];}
	     if((currMB->mb_y==0)&&(currMB->mb_x>0))
	     {max_L=LofCurrent[11*currMB->mb_y+currMB->mb_x-1];}
	      if (currMB->mb_x==10)
	     {max_L=LofCurrent[11*currMB->mb_y+currMB->mb_x-1];}
	  
	  if((currMB->mb_y!=0)||(currMB->mb_x!=0))
	  {if(max_L<=1)//slow class 2
	    {enc_mb.valid[1]=1; enc_mb.valid[0]=1;enc_mb.valid[I16MB]=1;enc_mb.valid[I4MB]=1;} 
	   if(max_L>1&&max_L<=2)//Moderate class 3 16x8 and 8x16
		   {enc_mb.valid[2]=1;
             enc_mb.valid[3]=1;
			 //enc_mb.valid[I16MB]=1;enc_mb.valid[I4MB]=1;
	       }
	   if(max_L>2)
	   {enc_mb.valid[P8x8]=1;enc_mb.valid[I16MB]=1;enc_mb.valid[I4MB]=1;}
	  }
  ...
  temp_mvx=0;
  temp_mvy=0;
  if(NumofFramebyLindan>1)
  {for(i_Lindan=0;i_Lindan<4;i_Lindan++)
   for(j_Lindan=0;j_Lindan<4;j_Lindan++)
   { temp_mvx+=p_Vid->enc_picture->mv_info[currMB->block_y+i_Lindan][currMB->block_x+j_Lindan].mv[LIST_0].mv_x;
    temp_mvy+=p_Vid->enc_picture->mv_info[currMB->block_y+i_Lindan][currMB->block_x+j_Lindan].mv[LIST_0].mv_y;
   }
  LofCurrent[11*currMB->mb_y+currMB->mb_x]=(double)(abs(temp_mvx)+abs(temp_mvy))/64;
  }
